...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p>Überführt eine zur EN 16931 konforme elektronische Rechnung in der konkreten
                Syntax UBL.2_1.Invoice in eine Instanz gemäß des Schemas für den Namensraum
    ...
    <xsl:template match="/Invoice:Invoice">
        <xr:invoice>
            ...
            <!--Manuell
            erstelltes Template für BG-1-->

            <!--Manuell
            erstelltes Template für BG-2-->
            <xr:PROCESS_CONTROL>
            ...
            angepasst für BG-16-->
            <!--PaymentMeansCode
            ist [1] in UBL und im semantischen Modell, daher kann auf diesem Attribut gruppiert
            werden-->
    ...
    <xsl:template mode="BG-3"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:BillingReference/cac:InvoiceDocumentReference der Instanz in
            konkreter Syntax wird auf 2 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-25"
                select="./cbc:ID" />
    ...
    <xsl:template mode="BG-4" match="/Invoice:Invoice/cac:AccountingSupplierParty">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:AccountingSupplierParty der Instanz in konkreter Syntax wird
            auf 10 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-27"
                select="./cac:Party/cac:PartyLegalEntity/cbc:RegistrationName" />
         <xsl:apply-templates
                mode="BT-28" select="./cac:Party/cac:PartyName/cbc:Name" />
         <xsl:apply-templates
                mode="BT-29" select="./cac:Party/cac:PartyIdentification/cbc:ID" />
         <xsl:apply-templates
                mode="BT-30" select="./cac:Party/cac:PartyLegalEntity/cbc:CompanyID" />
         <xsl:apply-templates
                mode="BT-31"
         ...
         <xsl:apply-templates
                mode="BT-34" select="./cac:Party/cbc:EndpointID" />
         <xsl:apply-templates mode="BG-5"
                select="./cac:Party/cac:PostalAddress" />
         <xsl:apply-templates mode="BG-6"
                select="./cac:Party/cac:Contact" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
            <xr:SELLER>
    ...
    <xsl:template mode="BG-5"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:AccountingSupplierParty/cac:Party/cac:PostalAddress der
            Instanz in konkreter Syntax wird auf 7 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-35"
                select="./cbc:StreetName" />
         <xsl:apply-templates mode="BT-36"
                select="./cbc:AdditionalStreetName" />
         <xsl:apply-templates mode="BT-162"
                select="./cac:AddressLine/cbc:Line" />
         <xsl:apply-templates mode="BT-37"
                select="./cbc:CityName" />
         <xsl:apply-templates mode="BT-38"
                select="./cbc:PostalZone" />
         <xsl:apply-templates mode="BT-39"
                select="./cbc:CountrySubentity" />
         <xsl:apply-templates mode="BT-40"
    ...
    <xsl:template mode="BG-6"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:AccountingSupplierParty/cac:Party/cac:Contact der Instanz in
            konkreter Syntax wird auf 3 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-41"
                select="./cbc:Name" />
         <xsl:apply-templates mode="BT-42" select="./cbc:Telephone" />
         <xsl:apply-templates
                mode="BT-43" select="./cbc:ElectronicMail" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
            <xr:SELLER_CONTACT>
    ...
    <xsl:template mode="BG-7" match="/Invoice:Invoice/cac:AccountingCustomerParty">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:AccountingCustomerParty der Instanz in konkreter Syntax wird
            auf 8 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-44"
                select="./cac:Party/cac:PartyLegalEntity/cbc:RegistrationName" />
         <xsl:apply-templates
                mode="BT-45" select="./cac:Party/cac:PartyName/cbc:Name" />
         <xsl:apply-templates
                mode="BT-46" select="./cac:Party/cac:PartyIdentification/cbc:ID" />
         <xsl:apply-templates
                mode="BT-47" select="./cac:Party/cac:PartyLegalEntity/cbc:CompanyID" />
            <!--<xsl:apply-templates
            mode="BT-47"
         ...
         <xsl:apply-templates
                ...
                select="./cac:Party/cac:PartyTaxScheme/cbc:CompanyID[following-sibling::cac:TaxScheme/cbc:ID = 'VAT']" />
         <xsl:apply-templates
                mode="BT-49" select="./cac:Party/cbc:EndpointID" />
         <xsl:apply-templates mode="BG-8"
                select="./cac:Party/cac:PostalAddress" />
         <xsl:apply-templates mode="BG-9"
                select="./cac:Party/cac:Contact" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template mode="BG-8"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:AccountingCustomerParty/cac:Party/cac:PostalAddress der
            Instanz in konkreter Syntax wird auf 7 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-50"
                select="./cbc:StreetName" />
         <xsl:apply-templates mode="BT-51"
                select="./cbc:AdditionalStreetName" />
         <xsl:apply-templates mode="BT-163"
                select="./cac:AddressLine/cbc:Line" />
         <xsl:apply-templates mode="BT-52"
                select="./cbc:CityName" />
         <xsl:apply-templates mode="BT-53"
                select="./cbc:PostalZone" />
         <xsl:apply-templates mode="BT-54"
                select="./cbc:CountrySubentity" />
         <xsl:apply-templates mode="BT-55"
                select="./cac:Country/cbc:IdentificationCode" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template mode="BG-9"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:AccountingCustomerParty/cac:Party/cac:Contact der Instanz in
            konkreter Syntax wird auf 3 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-56"
                select="./cbc:Name" />
         <xsl:apply-templates mode="BT-57" select="./cbc:Telephone" />
         <xsl:apply-templates
                mode="BT-58" select="./cbc:ElectronicMail" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
            <xr:BUYER_CONTACT>
    ...
    <xsl:template mode="BG-10" match="/Invoice:Invoice/cac:PayeeParty">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:PayeeParty der Instanz in konkreter Syntax wird auf 3 Objekte
            der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-59"
                select="./cac:PartyName/cbc:Name" />
         <xsl:apply-templates mode="BT-60"
                select="./cac:PartyIdentification/cbc:ID" />
         <xsl:apply-templates mode="BT-61"
    ...
    <xsl:template mode="BG-11" match="/Invoice:Invoice/cac:TaxRepresentativeParty">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:TaxRepresentativeParty der Instanz in konkreter Syntax wird
            auf 3 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-62"
                select="./cac:PartyName/cbc:Name" />
         <xsl:apply-templates mode="BT-63"
                select="./cac:PartyTaxScheme/cbc:CompanyID[following-sibling::cac:TaxScheme/cbc:ID = 'VAT']" />
         <xsl:apply-templates
                mode="BG-12" select="./cac:PostalAddress" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template mode="BG-12"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:TaxRepresentativeParty/cac:PostalAddress der Instanz in
            konkreter Syntax wird auf 7 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-64"
                select="./cbc:StreetName" />
         <xsl:apply-templates mode="BT-65"
                select="./cbc:AdditionalStreetName" />
         <xsl:apply-templates mode="BT-164"
                select="./cac:AddressLine/cbc:Line" />
         <xsl:apply-templates mode="BT-66"
                select="./cbc:CityName" />
         <xsl:apply-templates mode="BT-67"
                select="./cbc:PostalZone" />
         <xsl:apply-templates mode="BT-68"
                select="./cbc:CountrySubentity" />
         <xsl:apply-templates mode="BT-69"
                select="./cac:Country/cbc:IdentificationCode" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template mode="BG-13" match="/Invoice:Invoice/cac:Delivery">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:Delivery der Instanz in konkreter Syntax wird auf 5 Objekte
            der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-70"
                select="./cac:DeliveryParty/cac:PartyName/cbc:Name" />
         <xsl:apply-templates
                mode="BT-71" select="./cac:DeliveryLocation/cbc:ID" />
         <xsl:apply-templates
                mode="BT-72" select="./cbc:ActualDeliveryDate" />
            <!--<xsl:apply-templates
            mode="BG-14" select="/Invoice:Invoice/cac:InvoicePeriod"/>-->
         <xsl:apply-templates mode="BG-15"
                select="./cac:DeliveryLocation/cac:Address" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template mode="BG-14" match="/Invoice:Invoice/cac:InvoicePeriod">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:InvoicePeriod der Instanz in konkreter Syntax wird auf 2
            Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-73"
                select="./cbc:StartDate" />
    ...
    <xsl:template mode="BG-15"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:Delivery/cac:DeliveryLocation/cac:Address der Instanz in
            konkreter Syntax wird auf 7 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-75"
                select="./cbc:StreetName" />
         <xsl:apply-templates mode="BT-76"
                select="./cbc:AdditionalStreetName" />
         <xsl:apply-templates mode="BT-165"
                select="./cac:AddressLine/cbc:Line" />
         <xsl:apply-templates mode="BT-77"
                select="./cbc:CityName" />
         <xsl:apply-templates mode="BT-78"
                select="./cbc:PostalZone" />
         <xsl:apply-templates mode="BT-79"
                select="./cbc:CountrySubentity" />
         <xsl:apply-templates mode="BT-80"
                select="./cac:Country/cbc:IdentificationCode" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template mode="BG-17"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:PaymentMeans/cac:PayeeFinancialAccount der Instanz in
            konkreter Syntax wird auf 3 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-84"
                select="./cbc:ID" />
         <xsl:apply-templates mode="BT-85" select="./cbc:Name" />
         <xsl:apply-templates
                mode="BT-86" select="./cac:FinancialInstitutionBranch/cbc:ID" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
            <xr:CREDIT_TRANSFER>
    ...
    <xsl:template mode="BG-18" match="/Invoice:Invoice/cac:PaymentMeans/cac:CardAccount">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:PaymentMeans/cac:CardAccount der Instanz in konkreter Syntax
            wird auf 2 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-87"
                select="./cbc:PrimaryAccountNumberID" />
         <xsl:apply-templates mode="BT-88"
                select="./cbc:HolderName" />
        </xsl:variable>
    ...
    <xsl:template mode="BG-19"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:PaymentMeans/cac:PaymentMandate der Instanz in konkreter
            Syntax wird auf 3 Objekte der EN 16931 abgebildet. -->
         ...
         <xsl:apply-templates
                mode="BT-91" select="./cac:PayerFinancialAccount/cbc:ID" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
            <xr:DIRECT_DEBIT>
    ...
    <xsl:template mode="BG-20"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:AllowanceCharge[cbc:ChargeIndicator = 'false'] der Instanz in
            konkreter Syntax wird auf 7 Objekte der EN 16931 abgebildet. -->
    ...
    <xsl:template mode="BG-21"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:AllowanceCharge[cbc:ChargeIndicator = 'true'] der Instanz in
            konkreter Syntax wird auf 7 Objekte der EN 16931 abgebildet. -->
    ...
    <xsl:template mode="BG-22" match="/Invoice:Invoice/cac:LegalMonetaryTotal">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:LegalMonetaryTotal der Instanz in konkreter Syntax wird auf 10
            Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-106"
                select="./cbc:LineExtensionAmount" />
         <xsl:apply-templates mode="BT-107"
                select="./cbc:AllowanceTotalAmount" />
         <xsl:apply-templates mode="BT-108"
                select="./cbc:ChargeTotalAmount" />
         <xsl:apply-templates mode="BT-109"
                select="./cbc:TaxExclusiveAmount" />
         ...
         <xsl:apply-templates
                mode="BT-112" select="./cbc:TaxInclusiveAmount" />
         <xsl:apply-templates mode="BT-113"
                select="./cbc:PrepaidAmount" />
         <xsl:apply-templates mode="BT-114"
                select="./cbc:PayableRoundingAmount" />
         <xsl:apply-templates mode="BT-115"
                select="./cbc:PayableAmount" />
        </xsl:variable>
      <xsl:if test="$bg-contents">
            <xr:DOCUMENT_TOTALS>
    ...
    <xsl:template mode="BG-23" match="/Invoice:Invoice/cac:TaxTotal/cac:TaxSubtotal">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:TaxTotal/cac:TaxSubtotal der Instanz in konkreter Syntax wird
            auf 6 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-116"
                select="./cbc:TaxableAmount" />
         <xsl:apply-templates mode="BT-117"
         ...
         <xsl:apply-templates
                mode="BT-119" select="./cac:TaxCategory/cbc:Percent" />
         <xsl:apply-templates
                mode="BT-120" select="./cac:TaxCategory/cbc:TaxExemptionReason" />
         <xsl:apply-templates
                mode="BT-121" select="./cac:TaxCategory/cbc:TaxExemptionReasonCode" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
            <xr:VAT_BREAKDOWN>
    ...
    <xsl:template mode="BG-24" match="/Invoice:Invoice/cac:AdditionalDocumentReference">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:AdditionalDocumentReference der Instanz in konkreter Syntax
            wird auf 4 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-122"
                select="./cbc:ID" />
         <xsl:apply-templates mode="BT-123"
                select="./cbc:DocumentDescription" />
         <xsl:apply-templates mode="BT-124"
    ...
    <xsl:template mode="BG-25" match="/Invoice:Invoice/cac:InvoiceLine">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad /Invoice:Invoice/cac:InvoiceLine der Instanz in konkreter Syntax wird auf 15
            Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-126"
                select="./cbc:ID" />
         ...
         <xsl:apply-templates
            ...
            Code "130" MUST be used to indicate an invoice object reference. Not used for other
            additional documents -->
         <xsl:apply-templates
                mode="BT-129" select="./cbc:InvoicedQuantity" />
         ...
         <xsl:apply-templates mode="BT-132"
                select="./cac:OrderLineReference/cbc:LineID" />
         <xsl:apply-templates mode="BT-133"
                select="./cbc:AccountingCost" />
         <xsl:apply-templates mode="BG-26"
                select="./cac:InvoicePeriod" />
         ...
         <xsl:apply-templates
                ...
                select="./cac:AllowanceCharge[cbc:ChargeIndicator = 'true']" />
         <xsl:apply-templates
                mode="BG-29" select="./cac:Price" />
         <xsl:apply-templates mode="BG-30"
                select="./cac:Item/cac:ClassifiedTaxCategory" />
         <xsl:apply-templates mode="BG-31"
                select="./cac:Item" />
         <xsl:apply-templates mode="BG-DEX-01"
                select="cac:SubInvoiceLine" />
        </xsl:variable>
      <xsl:if test="$bg-contents">
            <xr:INVOICE_LINE>
    ...
    <xsl:template mode="BG-26" match="cac:InvoicePeriod">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:InvoicePeriod der Instanz in konkreter Syntax wird auf 2 Objekte der EN 16931
            abgebildet. -->
    ...
    <xsl:template mode="BG-27"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:AllowanceCharge[cbc:ChargeIndicator = 'false'] der Instanz in konkreter Syntax
            wird auf 5 Objekte der EN 16931 abgebildet. -->
    ...
    <xsl:template mode="BG-28"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:AllowanceCharge[cbc:ChargeIndicator = 'true'] der Instanz in konkreter Syntax
            wird auf 5 Objekte der EN 16931 abgebildet. -->
    ...
    <xsl:template mode="BG-29" match="cac:Price">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:Price der Instanz in konkreter Syntax wird auf 5 Objekte der EN 16931
            abgebildet. -->
         ...
         <xsl:apply-templates
                mode="BT-149" select="./cbc:BaseQuantity" />
         <xsl:apply-templates mode="BT-150"
                select="./cbc:BaseQuantity/@unitCode" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
            <xr:PRICE_DETAILS>
    ...
    <xsl:template mode="BG-30"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:Item/cac:ClassifiedTaxCategory der Instanz in konkreter Syntax wird auf 2
            Objekte der EN 16931 abgebildet. -->
    ...
    <xsl:template mode="BG-31" match="cac:Item">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:Item der Instanz in konkreter Syntax wird auf 8 Objekte der EN 16931
            abgebildet. -->
         <xsl:apply-templates mode="BT-153"
                select="./cbc:Name" />
         <xsl:apply-templates mode="BT-154" select="./cbc:Description" />
         <xsl:apply-templates
                mode="BT-155" select="./cac:SellersItemIdentification/cbc:ID" />
         <xsl:apply-templates
                mode="BT-156" select="./cac:BuyersItemIdentification/cbc:ID" />
         <xsl:apply-templates
                mode="BT-157" select="./cac:StandardItemIdentification/cbc:ID" />
         <xsl:apply-templates
                mode="BT-158"
                select="./cac:CommodityClassification/cbc:ItemClassificationCode" />
         <xsl:apply-templates
                mode="BT-159" select="./cac:OriginCountry/cbc:IdentificationCode" />
         <xsl:apply-templates
                mode="BG-32" select="./cac:AdditionalItemProperty" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
            <xr:ITEM_INFORMATION>
    ...
    <xsl:template mode="BG-32"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:Item/cac:AdditionalItemProperty der Instanz in konkreter Syntax wird auf 2
            Objekte der EN 16931 abgebildet. -->
    ...
    <xsl:template mode="BG-DEX-01" match="cac:SubInvoiceLine">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:SubInvoiceLine der Instanz in konkreter Syntax wird auf 14 Objekte der EN 16931
            abgebildet. -->
         <xsl:apply-templates mode="BT-126"
                select="./cbc:ID" />
         <xsl:apply-templates mode="BT-127" select="./cbc:Note" />
    ...
    <xsl:template mode="BG-DEX-02" match="cac:SubInvoiceLine/cac:Item">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:SubInvoiceLine/cac:Item der Instanz in konkreter Syntax wird auf 8 Objekte der
            EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-153"
         ...
         <xsl:apply-templates
                mode="BT-156" select="cac:Item/cac:BuyersItemIdentification/cbc:ID" />
         <xsl:apply-templates
                mode="BT-157" select="cac:Item/cac:StandardItemIdentification/cbc:ID" />
         <xsl:apply-templates
                mode="BT-158"
         ...
         <xsl:apply-templates
                mode="BG-DEX-08" select="cac:Item/cac:AdditionalItemProperty" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template mode="BG-DEX-08"
        match="cac:SubInvoiceLine/cac:Item/cac:AdditionalItemProperty">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:SubInvoiceLine/cac:Item/cac:AdditionalItemProperty der Instanz in konkreter
            Syntax wird auf 2 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-160"
                select="cac:Item/cac:AdditionalItemProperty/cbc:Name" />
         <xsl:apply-templates
                mode="BT-161" select="cac:Item/cac:AdditionalItemProperty/cbc:Value" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template mode="BG-DEX-03"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:SubInvoiceLine/cac:AllowanceCharge[cbc:ChargeIndicator = 'false'] der Instanz
            in konkreter Syntax wird auf 5 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-136"
                select="cac:AllowanceCharge/cbc:Amount[preceding-sibling::cbc:ChargeIndicator = 'false']" />
         <xsl:apply-templates
                mode="BT-137"
    ...
    <xsl:template mode="BG-DEX-04"
        ...
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:SubInvoiceLine/cac:AllowanceCharge[cbc:ChargeIndicator = 'true'] der Instanz in
            konkreter Syntax wird auf 5 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-141"
    ...
    <xsl:template mode="BG-DEX-05" match="cac:SubInvoiceLine/cac:InvoicePeriod">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:SubInvoiceLine/cac:InvoicePeriod der Instanz in konkreter Syntax wird auf 2
            Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-134"
    ...
    <xsl:template mode="BG-DEX-06" match="cac:SubInvoiceLine/cac:ClassifiedTaxCategory">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:SubInvoiceLine/cac:ClassifiedTaxCategory der Instanz in konkreter Syntax wird
            auf 2 Objekte der EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-151"
                select="cac:Item/cac:ClassifiedTaxCategory/cbc:ID" />
         <xsl:apply-templates
                mode="BT-152" select="cac:Item/cac:ClassifiedTaxCategory/cbc:Percent" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template mode="BG-DEX-07" match="cac:SubInvoiceLine/cac:Price">
        <xsl:variable name="bg-contents" as="item()*"><!--Der
            Pfad cac:SubInvoiceLine/cac:Price der Instanz in konkreter Syntax wird auf 5 Objekte der
            EN 16931 abgebildet. -->
         <xsl:apply-templates mode="BT-146"
         ...
         <xsl:apply-templates
                mode="BT-149" select="cac:Price/cbc:BaseQuantity" />
         <xsl:apply-templates
                mode="BT-150" select="cac:Price/cbc:BaseQuantity/@unitCode" />
        </xsl:variable>
      <xsl:if
            test="$bg-contents">
    ...
    <xsl:template name="identifier">
      ...
      <xsl:if
            test="@schemeVersionID | @listVersionID">
