...
public final class CertificateVerifier {
	 ...
	 * @param verifySelfSignedCert true if a self-signed certificate is accepted,
	 *                             false if not.
	...
	public static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert,
			Set<X509Certificate> additionalCerts, boolean verifySelfSignedCert, Date signDate)
		...
		try {
			...
			while (!certsToTrySet.isEmpty()) {
				...
				for (X509Certificate tryCert : certsToTrySet) {
					Set<X509Certificate> downloadedExtraCertificatesSet = CertificateVerifier
							.downloadExtraCertificates(tryCert);
			...
			PKIXCertPathBuilderResult verifiedCertChain = verifyCertificate(cert, trustAnchors, intermediateCerts,
					signDate);
	...
	private static void checkRevocations(X509Certificate cert, Set<X509Certificate> additionalCerts, Date signDate)
			throws IOException, CertificateVerificationException, OCSPException, RevokedCertificateException,
		...
		// issuerCert is never null here. If it hadn't been found, then there wouldn't
		// be a
	 ...
	 * "authority information access" extension. The method is lenient, i.e. catches
	 * all exceptions.
	 *
	 ...
	 *                                  (e.g. certification path cannot be built or
	 *                                  some certificate in the chain is expired)
	 */
	private static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert, Set<TrustAnchor> trustAnchors,
		...
		// requested target"
		// (when using -Djava.security.debug=certpath: "critical policy qualifiers
		// present in certificate")
		// for files like 021496.pdf that have the "Adobe CDS Certificate Policy"
		...
		// "If the policyQualifiersRejected flag is set to false, it is up to the
		// application
		// to validate all policy qualifiers in this manner in order to be PKIX
		// compliant."
	...
	/**
	 * Verify whether the certificate has been revoked at signing date, and verify
	 * whether the certificate of the responder has been revoked now.
	 *
	...
	private static void verifyOCSP(OcspHelper ocspHelper, Set<X509Certificate> additionalCerts)
		...
		if (ocspResponse.getStatus() != OCSPResp.SUCCESSFUL) {
			throw new CertificateVerificationException(
					"OCSP check not successful, status: " + ocspResponse.getStatus());
		}
