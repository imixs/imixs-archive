...
public class SigUtils {
	 ...
	 * document in the DocMDP transform parameters dictionary. Details are described
	 * in the table "Entries in the DocMDP transform parameters dictionary" in the
	 * PDF specification.
	 *
	...
	/**
	 * Log if the certificate is not valid for signature usage. Doing this anyway
	 * results in Adobe Reader failing to validate the PDF.
	 *
	...
	public static PDSignature getLastRelevantSignature(PDDocument document) throws IOException {
		...
		// we can't use getLastSignatureDictionary() because this will fail (see
		// PDFBOX-3978)
		// if a signature is assigned to a pre-defined empty signature field that isn't
		// the last.
		// we get the last in time by looking at the offset in the PDF file.
		Optional<PDSignature> optLastSignature = document.getSignatureDictionaries().stream()
				.sorted(comparatorByOffset.reversed()).findFirst();
	...
	/**
	 * Verify the certificate chain up to the root, including OCSP or CRL. However
	 * this does not test whether the root certificate is in a trusted list.<br>
	 * <br>
