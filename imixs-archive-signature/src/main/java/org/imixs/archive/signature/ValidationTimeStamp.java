...
/**
 * This class wraps the TSAClient and the work that has to be done with it. Like
 * Adding Signed TimeStamps to a signature, or creating a CMS timestamp
 * attribute (with a signed timestamp)
 *
 * @author Others
 * @author Alexis Suter
 */
public class ValidationTimeStamp {
	...
	public CMSSignedData addSignedTimeStamp(CMSSignedData signedData) throws IOException {
		...
		// Because new SignerInformation is created, new SignerInfoStore has to be
		// created
	...
	private SignerInformation signTimeStamp(SignerInformation signer) throws IOException {
		...
		// There is no other way changing the unsigned attributes of the signer
		// information.
