...
A _snapshot workitem_ is a copy of the current workitem (origin-workitem) including all the file content of attached files. The origin-workitem holds a reference of IDs to each snapshot-workitem generated by the Archive API (_$snapshotID_). The _$snapshotID_ can be used to load attached file data ot of the snapshot-workitem. Attached files will be linked from the snapshot-workitem to the origin-workitem.
